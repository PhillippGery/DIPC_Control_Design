function [T_log, X_log, U_log, X_hat_log] = run_mpc_sim_with_Observer(mpc_obj,f_handle_3u, Ad, Bd, Cd, C_u3, x_init, Ld, h, tfinal, r_target, xe, ue, video_filename)
% This function simulates a discrete-time linear plant controlled by a
% discrete-time MPC controller (which includes its own observer).
%
% Inputs:
%   mpc_obj      - The MPC controller object
%   Ad, Bd, Cd   - Discrete-time linear model matrices (the "plant")
...
disp('Starting DISCRETE plant simulation with MPC...');
% --- Simulation Setup ---
t_steps = floor(tfinal / h); % Number of simulation steps

% Initialize the MPC state object
mpc_state = mpcstate(mpc_obj);
% Set the initial state of the internal observer to match the
% true initial deviation of the plant from the equilibrium
...
for k = 1:t_steps
    ...
    % 1. Measure the "true" plant output
    y_true = C_u3 * x_true;
    
    % 2. Calculate control move. This ALSO updates mpc_state (the observer).
    ...
    % Get the observer's state from the updated mpc_state object
    x_hat = mpc_state.Plant + xe; 
    X_hat_log(k+1, :) = x_hat';
end
...
title('MPC Control of DISCRETE DIPC Model');
cart_width = 0.5;
cart_height = 0.25;
y_cart = 0;
x_cart = X_log(1, 1);
